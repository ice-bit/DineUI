...
class EditTableFormViewController: UIViewController {
    ...
    @objc private func saveButtonAction(_ sender: UIButton) {
        ...
        guard isLocationIdAvailable(locationId) else {
    ...
    private func isLocationIdAvailable(_ locationId: Int) -> Bool {
        ...
        do {
            ...
            if let resultTables {
                if resultTables.first(where: { $0.locationIdentifier ==  locationId }) != nil {
                    return false
                }
            }
        } catch {
            fatalError("failed to perform fetch op: \(error)")
        }
    ...
    private func validateInput(_ textField: DineTextField) -> DineTextInputError? {
        ...
        if textField == locIdTextField {
            if let locationId = Int(text) {
                if !isLocationIdAvailable(locationId) {
                    return DineTextInputError(localizedDescription: "Location ID already exists.")
                }
            }
        }
